...
export default function Register() {
  ...
  useEffect(() => {
    ...
    const eConfirm = validateConfirm(form.password, form.confirm);
    if (eConfirm) errs.confirm = eConfirm;
    const eAlias = validateAlias(form.alias);
    if (eAlias) errs.alias = eAlias;
    setErrors(errs);
  }, [form]);
    ...
    <div className="container" style={{ paddingTop: 24, paddingBottom: 48 }}>
      <div className="card" style={{ maxWidth: 600, margin: "0 auto" }}>
        ...
        <p style={{ color: "var(--muted)" }}>
          Vista previa <strong>sin backend</strong>. Regla de contraseña: 8–12,
          alfanumérica, con al menos una minúscula y una mayúscula.
        </p>
        ...
        <form className="form" noValidate onSubmit={handleSubmit}>
          ...
          <div className="form__group">
            ...
            />
            {showError("name") && (
              <div id="err-name" className="error">
                {errors.name}
              </div>
            )}
          </div>
          ...
          <div className="form__group">
            ...
            <input
              ...
              aria-describedby={
                showError("email") ? "err-email help-email" : "help-email"
              }
            />
            {showError("email") && (
            ...
            <div id="help-email" className="help">
              El correo debe ser único (se validará en el backend).
            </div>
          </div>
          ...
          <div className="form__group">
            ...
            <div className="meter" aria-hidden="true">
              <div
                ...
                style={{ width: `${passRules.percent}%` }}
              />
            </div>
            <div className="help">
              Estado: <strong>{passRules.label}</strong>
            </div>

            <ul id="req-pass" className="requirements">
              <li className={passRules.lenOk ? "ok" : "bad"}>
                8–12 caracteres
              </li>
              <li className={passRules.hasLower ? "ok" : "bad"}>
                Al menos una minúscula
              </li>
              <li className={passRules.hasUpper ? "ok" : "bad"}>
                Al menos una mayúscula
              </li>
              <li className={passRules.onlyAlnum ? "ok" : "bad"}>
                Solo letras y números
              </li>
            </ul>

            {showError("password") && (
              <div id="err-password" className="error">
                {errors.password}
              </div>
            )}
          </div>
          ...
          <div className="form__group">
            ...
            <div className="input-group">
              <input
                ...
                aria-describedby={
                  showError("confirm") ? "err-confirm" : undefined
                }
            ...
            {showError("confirm") && (
              <div id="err-confirm" className="error">
                {errors.confirm}
              </div>
            )}
          </div>
          ...
          <div className="form__group">
            ...
            />
            {showError("alias") && (
              <div id="err-alias" className="error">
                {errors.alias}
              </div>
            )}
          </div>
