...
const PASS_NOTE = 'Solo visual: la unicidad de nombre la valida el backend real.'
...
export default function SeasonCreate() {
  ...
  useMemo(() => {
    ...
    for (let i = 0; i < slots; i++) {
      ...
      gen.push({ start: new Date(wStart).toISOString().slice(0,10), end: new Date(wEnd).toISOString().slice(0,10) })
      cur = wEnd
    }
    setWeeks(gen)
  ...
  function setWeek(i, key, val) {
    setWeeks(ws => ws.map((w, idx) => (idx === i ? { ...w, [key]: val } : w)))
    setToast({ type: null, message: '' })
  }

  function validateAll() {
    const e = {}
    const eName = validateSeasonName(form.name); if (eName) e.name = eName
    const eCnt = validateWeeksCount(form.weeksCount); if (eCnt) e.weeksCount = eCnt
    const eDates = validateSeasonDates(form.startDate, form.endDate); if (eDates) e.dates = eDates
    const eWeeks = validateWeeksInRange(weeks, form.startDate, form.endDate); if (eWeeks) e.weeks = eWeeks
    setErrors(e)
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fakeSeasonCreateVisual({
        ...generated,
        name: form.name.trim(),
      ...
      setSubmitting(false)
      setToast({ type: 'ok', message: `Temporada creada: ${form.name}. Semanas: ${form.weeksCount}.` })
      // No dependemos de página admin real; si quieres, quédate aquí:
      // return
      setTimeout(() => navigate(res.data.redirect_to), 900)
    } catch (err) {
      setSubmitting(false)
      setToast({ type: 'err', message: err?.message || 'No se pudo crear la temporada.' })
    }
  }
        ...
        <form className="form" onSubmit={handleSubmit} noValidate>
          <div className="grid" style={{ gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 16 }}>
            ...
            <div className="form__group">
              ...
              />
              {errors.weeksCount && <div className="error">{errors.weeksCount}</div>}
            </div>
          ...
          <div className="card" style={{ marginTop: 12 }}>
            ...
            {errors.weeks && <div className="error" style={{ marginBottom: 8 }}>{errors.weeks}</div>}
            <div className="grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
              {weeks.map((w, i) => (
                <div key={i} className="card" style={{ padding: 8 }}>
                  <div className="help" style={{ marginBottom: 4 }}>Semana {i + 1}</div>
