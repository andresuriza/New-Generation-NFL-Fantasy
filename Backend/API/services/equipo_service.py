...
class EquipoNFLService:
    ...
    @handle_db_errors
    def crear_equipo(self, equipo: EquipoNFLCreate) -> EquipoNFLResponse:
        """Create a new NFL team"""
        # Use validator for name uniqueness check
        validator = EquipoNFLValidator()
        validator.validate_nombre_unique(equipo.nombre)
        
        nuevo_equipo = equipo_repository.create(equipo)
        return _to_response(nuevo_equipo)

    def listar(self) -> List[EquipoNFLResponse]:
        """List all NFL teams"""
        items = equipo_repository.list_all()
        return [_to_response(e) for e in items]

    def obtener(self, equipo_id: UUID) -> EquipoNFLResponse:
        """Get NFL team by ID"""
    ...
    def obtener_con_media(self, equipo_id: UUID) -> EquipoNFLConMedia:
        """Get NFL team by ID with media information"""
        equipo = equipo_repository.get_with_media(equipo_id)
        if not equipo:
            raise NotFoundError("Equipo NFL no encontrado")
        return _to_response_con_media(equipo)

    @handle_db_errors
    def actualizar(self, equipo_id: UUID, equipo_update: EquipoNFLUpdate) -> EquipoNFLResponse:
        """Update NFL team"""
        validator = EquipoNFLValidator()
        equipo_existente = validator.validate_exists(equipo_id)
        
        # Check if new name already exists (if name is being changed)
        if equipo_update.nombre and equipo_update.nombre != equipo_existente.nombre:
            validator.validate_nombre_unique(equipo_update.nombre, equipo_id)
        
        equipo_actualizado = equipo_repository.update(equipo_id, equipo_update)
    ...
    def eliminar(self, equipo_id: UUID) -> None:
        ...
        equipo_repository.delete(equipo_id)
